...
app.init = function() {
    ...
    //配置导航条的动画
    app.configIntAnimate();

    app.button3D(".start", ".state1", ".state2", 0.3);
    app.button3D(".button1", ".state1", ".state2", 0.3);
    app.button3D(".button2", ".state1", ".state2", 0.3);

    //设置每一屏中img的百分比
    app.imgWidth($(".scene img"));

    //配置横屏切换的动画
    app.configTimeScroll();
...
app.events = function() {
    ...
    $(window).bind("scroll", app.scrollStatus);
    ...
    $(window).bind("mouseup", app.mouseupFn)
...
//当mouseup的时候，让当前这一屏到达某个状态
app.mouseupFn = function() {
    //在滚动过程中计算出一个比例
    var scale = app.scale();

    //得到当前页面到达的某个时间点
    var times = scale * app.timeScroll.totalDuration();

    //获取到上一个状态和下一个状态
    var prevStep = app.timeScroll.getLabelBefore(times);
    var nextStep = app.timeScroll.getLabelAfter(times);

    //获取到上一个状态的时间和下一个状态的时间
    var prevTime = app.timeScroll.getLabelTime(prevStep);
    var nextTime = app.timeScroll.getLabelTime(nextStep);
    ...
    */
    ...
    app.timeScroll.tweenTo(step);

    //-----------------当松开鼠标时候，控制滚动条到达某个状态计算出的距离-----------------------
    //获取动画的总时长
    var totalTime = app.timeScroll.totalDuration();

    //获取到要到达的状态的时间
    var afterTime = app.timeScroll.getLabelTime(step);
    ...
    //滚动条滚动的距离的持续时间
    var d = Math.abs(app.timeScroll.time() - afterTime);
...
//在滚动条滚动的过程中，计算页面中应该到哪一个时间点上去
app.scrollStatus = function() {
    var times = app.scale() * app.timeScroll.totalDuration();
    //当滚动条在滚动的过程中，让页面中的动画到打某个时间点
    app.timeScroll.seek(times, false);
}

//切换整屏并且计算滚动条的距离
app.changeStep = function(value) {
    //向下切换
    if (value === "next") {
        //获取当前的时间
        var currentTime = app.timeScroll.getLabelTime(app.currentStep);

        //获取到下一个状态的字符串
        var afterStep = app.timeScroll.getLabelAfter(currentTime);
        ...
        //获取到下一个状态的时间
        var afterTime = app.timeScroll.getLabelTime(afterStep);
        ...
        //获取到上一个状态的字符串
        var beforeStep = app.timeScroll.getLabelBefore(currentTime);
        ...
        //获取到下一个状态的时间
        var BeforeTime = app.timeScroll.getLabelTime(beforeStep);
        ...
        //滚动条滚动的距离的持续时间
        var d = Math.abs(app.timeScroll.time() - BeforeTime);
        var scrollAnimate = new TimelineMax();

        scrollAnimate.to("html,body", d, { scrollTop: positionY });

        //记录当前的状态为上一个状态，方便继续切换到上一个状态上
        app.currentStep = beforeStep;
    }
}

//配置整屏切换的动画以及每一屏中的小动画
app.configTimeScroll = function() {

    var time = app.timeScroll ? app.timeScroll.time() : 0;
    if (app.timeScroll) {
        app.timeScroll.clear();
    }

    app.timeScroll = new TimelineMax();

    // 当从第二屏切换到第一屏的时候，让第二屏里面的动画时间点重归0
    app.timeScroll.to(".scene1", 0, {
        onReverseComplete: function() {
            twoAnimate.timeline.seek(0, false);
        }
    }, 0);

    app.timeScroll.to(".footer", 0, { top: "100%" });

    app.timeScroll.add("step1");
    app.timeScroll.to(".scene2", 0.8, { top: 0, ease: Cubic.easeInOut });
    app.timeScroll.to({}, 0.1, {
        onComplete: function() {
    ...
    app.timeScroll.to({}, 0.4, {});
    app.timeScroll.add("point1");
    app.timeScroll.to({}, 0, {
        onComplete: function() {
    ...
    app.timeScroll.to({}, 0.4, {});
    app.timeScroll.add("point3");

    // --- 主动画中配置第二萍的小动画 end
    app.timeScroll.to(".scene3", 0.8, {
        top: 0,
    ...
    app.timeScroll.to({}, 0.1, {
        onComplete: function() {
        	//切换到第二屏调用的函数，同时传入导航条背景颜色变化的class名字
    ...
    app.timeScroll.to({}, 0.4, {});
    app.timeScroll.add("threeSate");

    // --- 主动画中配置第三屏的小动画 end
    app.timeScroll.to(".scene4", 0.8, { top: 0, ease: Cubic.easeInOut });
    app.timeScroll.add("step4");

    //滚动到第五屏的时候，要让第四屏滚出屏幕外
    app.timeScroll.to(".scene4", 0.8, { top: -$(window).height(), ease: Cubic.easeInOut });
    //当可视区域大于950，那么就让导航条隐藏起来
    if ($(window).width() > 950) {
        app.timeScroll.to(".menu_wrapper", 0.8, { top: -110, ease: Cubic.easeInOut }, "-=0.8");
    } else {
        $(".menu_wrapper").css("top", 0);
    }

    app.timeScroll.to(".scene5", 0.8, {
        top: 0,
        ease: Cubic.easeInOut,
        onReverseComplete: function() {
    ...
    app.timeScroll.add("step5");

    app.timeScroll.to(".scene5", 0.5, { top: -$(".footer").height(), ease: Cubic.easeInOut });
    app.timeScroll.to(".footer", 0.5, { top: $(window).height() - $(".footer").height(), ease: Cubic.easeInOut }, "-=0.5");

    app.timeScroll.add("footer");

    app.timeScroll.stop();
    //当改变浏览器的大小时，让动画走到之前已经到达的时间点
    app.timeScroll.seek(time);
}

//配置导航条的动画
app.configIntAnimate = function() {
...
// 3D翻转效果
app.button3D = function(obj, element1, element2, d) {
...
//设置img的百分比
app.imgWidth = function(elementImg) {
